...
bool JDevice::isDeviceSuitable(VkPhysicalDevice device){
    ...
    // vkGetPhysicalDeviceProperties(device, &physicalDeviceProperties_); //copy the info
...
int JDevice::rateDeviceSuitability(VkPhysicalDevice device){
    ...
    vkGetPhysicalDeviceProperties(device, &physicalDeviceProperties_); 
    VkPhysicalDeviceFeatures deviceFeatures;
    vkGetPhysicalDeviceFeatures(device, &deviceFeatures);

    int score=0;
    //discrete GPU high score
    if(physicalDeviceProperties_.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU){  score += 1000; }

    score += physicalDeviceProperties_.limits.maxImageDimension2D;  // support bigger 2d image resolution, score higher
...
VkResult JDevice::createBuffer(VkBufferUsageFlags usageFlags, VkMemoryPropertyFlags memoryPropertyFlags, tools::Buffer *buffer, VkDeviceSize size, void *data)
	{
		...
		VK_CHECK_RESULT(vkCreateBuffer(device_, &bufferInfo, nullptr, &buffer->buffer));
        ...
        allocInfo.memoryTypeIndex = util::findMemoryType(memReqs.memoryTypeBits, memoryPropertyFlags , physicalDevice_ );		
		...
		VK_CHECK_RESULT(vkAllocateMemory(device_, &allocInfo, nullptr, &buffer->memory));
